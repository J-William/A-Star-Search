...
class Problem:
    ...
    class Node(object):
        ...
        def __repr__(self) -> str:
            ...
            return self.state.__repr__() + " Action: {}, pathCost: {}".format(
                self.action, self.pathCost
            )
    ...
    class Frontier:
        ...
        def replace(self, node: Problem.Node) -> None:
            ...
            for i in range(len(self.array)):
                if (self.array[i][1].state == node.state) and (
                    self.array[i][0] > node.pathCost
                ):
                    # Replace
    ...
    def result(self, state: State, action: Action) -> list:
        ...
        newState.array[blank], newState.array[target] = (
            newState.array[target],
            newState.array[blank],
        )
        return newState
